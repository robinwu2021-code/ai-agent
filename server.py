...
_BI_REPORT_SYSTEM_PROMPT = """## BI 报表分析规则

### 严禁生成图表代码
- **禁止**生成 matplotlib、pyplot、plotly、seaborn 或任何 Python 图表代码。
- **禁止**调用 `execute_python` 工具生成或展示图表。
- 前端已内置 Recharts 图表渲染器，所有图表将由前端自动生成，无需后端输出任何代码。

### 必须输出 JSON 数据块
查询到数据后，**在回复文字之前**，先输出如下格式的 JSON 块（前端识别后自动渲染图表）：

```json
{
  "metrics": [
    {"period_label": "本周", "turnover": 6012.74, "order_quantity": 82},
    {"period_label": "上周", "turnover": 46254.31, "order_quantity": 479}
  ],
  "aggregation": "SUM",
  "bra_id": "B17612377308779358",
  "range_start": 1700000000000,
  "range_end": 1700086400000
}
```

### aggregation 参数规则（重要）

- **不传 aggregation** → API 返回时间区间内**每天一行**的 list，适合趋势/明细查询。
- **传 aggregation=SUM** → API 返回**单行汇总**，适合"今天总营业额"、"本周合计"等场景。

**选择依据：**
| 用户意图 | aggregation | 示例 |
|---------|------------|------|
| 今天/某天的总营业额 | SUM | "今天卖了多少" |
| 本周/本月合计 | SUM | "这个月总收入" |
| 每天趋势、日明细 | 不传 | "上周每天怎么样" |
| 按月汇总趋势 | 不传 + timeGranularity=MONTH | "近半年每月营业额" |

### 逐日 / 多周期查询规则

**核心规则：一次 agent_bi 调用即可获得范围内每天的数据列表。**

- 用户要查"上周每天"、"日趋势"等：调用 agent_bi **一次**，传入整个时间段，**不传 aggregation**，API 自动返回每天一行的数据列表。
- 每行数据已自动带有 `period_label`（如 "12/14(周一)"），前端直接渲染为折线图 X 轴标签。
- 用户要对比不同周期（如"本周 vs 上周"）：分别调用两次 agent_bi，各自传入对应时间段，再把结果合并到一个 `metrics` 数组，用 `period_label` 区分（如"本周"、"上周"）。
- **绝对禁止**用 Python/matplotlib 模拟或估算数据——所有数据必须来自真实 API 调用。

### 空数据处理规则（重要）
- 若 agent_bi 返回 `"no_data": true`，说明该时间段确实没有数据，**立即回复用户"暂无数据"，禁止重试或再次调用工具**。
- 不得因为 metrics 为空就反复调用 agent_bi，一次调用结果为空即为最终答案。

### 同比 / 环比查询规则
- 用户要求"同比"时，在 `agent_bi` 调用中设置 `comparison="yoy"`；要求"环比"时设置 `comparison="pop"`。
- 工具会自动返回对比数据，无需手动分别查询当期和对比期。
- 返回的 JSON 块中包含 `comparison_type`、`previous_metrics`、`deltas` 字段，前端会自动渲染对比图表。

### 语言支持
- 支持中文、英文、阿拉伯语等多种语言响应，根据用户提问语言自动匹配回复语言。
- `period_label` 使用与用户问题相同的语言（中文问则用中文标注，英文问则用英文标注）。
"""
...
def _build_prompt(req: "ChatRequest") -> str | None:
    ...
    if is_bi_request:
        ...
        return _BI_REPORT_SYSTEM_PROMPT + (base or "") + bi_ctx
...
async def _stream_agent(req: ChatRequest) -> AsyncIterator[str]:
    ...
    async for ev in container.agent().run(
        ...
        # 当 agent_bi 工具调用完成时，额外推送一个 bi_data 事件。
        # 这样前端无需依赖 LLM 在回复文字中输出 JSON 块就能渲染图表。
        if (
            ev.get("type") == "step"
            and ev.get("tool") == "agent_bi"
            ...
            if isinstance(result, dict) and result.get("success") and "metrics" in result:
                ...
                yield f"data: {json.dumps(bi_payload, ensure_ascii=False)}\n\n"
